  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/gym/register", {
        ...
        body: JSON.stringify({ ...formData, tier: "pro" }), // Default to pro tier
      });
      ...
      // Store admin credentials in sessionStorage for display
      if (data.admin) {
      ...
      router.push(`/gym-portal/gym-signup/success?gymId=${data.gym.id}`);
    } catch (err) {
    ...
    <div className="py-12 lg:py-20">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          >
            {isSubmitting ? "Registracija u toku..." : "Registruj teretanu"}
          </button>
        </form>
      </div>
    </div>
